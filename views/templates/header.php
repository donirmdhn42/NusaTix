<?php
// Pastikan sesi sudah dimulai di file utama
if (session_status() === PHP_SESSION_NONE) {
    session_start();
}

require_once __DIR__ . '/../../backend/helpers/functions.php';
// Memuat fungsi requireAdmin dari auth.php
...
<style>
    /* Gaya untuk item navigasi mobile yang aktif */
    .mobile-nav-link.mobile-nav-active .nav-icon-wrapper {
        background-color: rgba(233, 41, 50, 0.15);
        /* Latar belakang lingkaran merah transparan */
    }

    .mobile-nav-link.mobile-nav-active .nav-icon,
    .mobile-nav-link.mobile-nav-active .nav-text {
        color: #e92932;
        /* Warna ikon dan teks menjadi merah */
    }

    .mobile-nav-link.mobile-nav-active .nav-text {
        font-weight: 600;
        /* Teks menjadi sedikit tebal */
    }

    /* Mencegah flicker pada dropdown lokasi saat refresh */
    [x-cloak] {
...
<header class="hidden md:flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[#382929] px-10 py-3 text-white sticky top-0 bg-[#181111] z-20">
    <div class="flex-1">
        <a href="index.php" class="flex items-center gap-4">
            <div class="size-4 text-red-500"><svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.location-link-mobile').forEach(link => {
            link.addEventListener('click', function(e) {
                ...
                updateLocationDisplay();
                // Close the modal using AlpineJS's event system
                document.dispatchEvent(new CustomEvent('close-location-modal'));
            });
        });

        // Listener untuk menutup modal dari event
        document.addEventListener('close-location-modal', () => {
            // This leverages Alpine's reactivity.
            // We find the component and set its data property to false.
        ...
        // --- LOGIKA NAVIGASI AKTIF MOBILE ---
        const currentPage = window.location.pathname.split('/').pop() || 'index.php';
        // Match specific auth pages to their icons
